...
begin
  ...
  with QData do begin
    ...
    if tipekontak = -1 then begin
      SQL.Text := 'select *,case when tipekontak=0 then '+QuotedStr('Customer')+' when tipekontak=1 then '+QuotedStr('Supplier')+' else '+QuotedStr('Sales')+' end as Pos from tbl_kontak where aktif=1 order by namakontak';
    end else begin
      SQL.Text := 'select *,case when tipekontak=0 then '+QuotedStr('Customer')+' when tipekontak=1 then '+QuotedStr('Supplier')+' else '+QuotedStr('Sales')+' end as Pos from tbl_kontak where tipekontak=:tp and aktif=1 order by namakontak';
...
begin
  ...
  {if QData.FieldValues['blacklist'] = 1 then begin
    MessageDlg(RzPanel2.Caption+' di BLACKLIST. Data tidak dapat digunakan!',mtError,[mbOK],0);
    Exit;
  end;  }
  if fungsiblok = 1 then begin
    if QData.FieldValues['tipekontak'] = 0 then begin
      if QData.FieldValues['blacklist'] = 1 then begin
        if dm.CekCustomerOverdo(QData.FieldValues['nokontak']) = False then begin
          MessageDlg('Customer '+QData.FieldValues['namakontak']+' memiliki Piutang Jatuh Tempo diatas 30 hari! Data tidak dapat digunakan!',mtError,[mbOK],0);
          Exit;
        end;
      end;
    end;
