...
type
  TFrm_InputKontak = class(TForm)
    ...
    procedure txtrshButtonClick(Sender: TObject);
    procedure txtrshKeyPress(Sender: TObject; var Key: Char);
  private
...
uses UDM, UDataAkun, UMain, UDataKontak;
...
procedure TFrm_InputKontak.txtrshButtonClick(Sender: TObject);
begin
  ...
  if Frm_DataKontak.ShowModal = mrok then begin
    txtrsh.Text := Frm_DataKontak.QData.FieldValues['namakontak'];
    LNoRSH.Caption := Frm_DataKontak.QData.FieldValues['nokontak'];
  end;
end;

procedure TFrm_InputKontak.txtrshKeyPress(Sender: TObject; var Key: Char);
begin
  txtrshButtonClick(nil);
end;
