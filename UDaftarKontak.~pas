...
begin
  ...
  if Frm_InputKontak.ShowModal = mrok then begin
    ...
    with TZQuery.Create(Self)do begin
      Connection := dm.Con;
      Close;
      SQL.Text := 'insert into tbl_kontak values (:a,:b,:c,:d,:e,:f,:g,:h,:i,:j,:k,:l,:m,:n,:o,:p,:q,:r,:s,:t,:u,:v,:w,:x,:y,:z,:aa,:ab,:ac,:ad,:ae,:af,:ag,:ah,:ai,:aj,:ak,:al)';
      ...
      if Frm_InputKontak.CBBlacklist.Checked = True then
        ParamByName('ah').Value := 1
      else ParamByName('ah').Value := 0;
      if Frm_InputKontak.cbaktif.Checked = True then
        ParamByName('ai').Value := 1
      else ParamByName('ai').Value := 0;
      ParamByName('aj').Value := FormatDateTime('yyyy-mm-dd',tanggalserver);
      ParamByName('ak').Value := Frm_Main.txtuser.Caption;
      if Frm_InputKontak.cbtipe.ItemIndex = 0 then
        ParamByName('al').Value := Frm_InputKontak.LNoRSH.Caption
      else
        ParamByName('al').Value := 0;
      ExecSQL;
...
begin
  ...
  with Frm_InputKontak do begin
    ...
    with TZQuery.Create(Self)do begin
      ...
      if FieldValues['blacklist']=1 then
        Frm_InputKontak.CBBlacklist.Checked := True
      else Frm_InputKontak.CBBlacklist.Checked := False;
      if FieldValues['aktif']=1 then
        Frm_InputKontak.cbaktif.Checked := True
      else Frm_InputKontak.cbaktif.Checked := False;
      LNoRSH.Caption := FieldValues['rsh'];
      ...
      if LNoRSH.Caption <> '0' then  begin
        Close;
        SQL.Clear;
        SQL.Text := 'select * from tbl_kontak where nokontak=:ib';
        ParamByName('ib').Value := LNoRSH.Caption;
        Open;
        txtrsh.Text := FieldValues['namakontak'];
      end;
      Close;
      SQL.Clear;
      ...
      if Frm_InputKontak.ShowModal = mrok then begin
        ...
        SQL.Text := 'update tbl_kontak set kodekontak=:b,namakontak=:c,namanpwp=:cc,noakun=:d,tipekontak=:e,npwp=:f,plafon=:g,telpon1kontak=:h,telpon2kontak=:i,faxkontak=:j,' +
                    ...
                    'nilaipenjualan=:t,diskonday=:v,duedate=:w,earlydiskon=:x,latecharge=:y,diskon=:z,field1=:aa,field2=:ab,pembayaran=:ac,noakunpotongan=:ad,blacklist=:ah,aktif=:ai,rsh=:al where nokontak=:a';
...
begin
  ...
  if QData.FieldValues['tipekontak'] = 1 then begin
    ...
    Frm_InputSaldoAwalHutang.LHutang.Caption := QData.FieldValues['noakun'];
    if Frm_InputSaldoAwalHutang.ShowModal = mrok then begin
      with TZQuery.Create(Self)do begin
        ...
        try
          ...
          ParamByName('b').Value := Frm_InputSaldoAwalHutang.txtpembelian.Text;
          ParamByName('c').Value := FormatDateTime('yyyy-mm-dd',Frm_InputSaldoAwalHutang.dtpfaktur.Date);
          ParamByName('d').Value := 0;
          ParamByName('e').Value := Frm_InputSaldoAwalHutang.LKontak.Caption;
          ParamByName('f').Value := Frm_InputSaldoAwalHutang.LHutang.Caption;
          ParamByName('h').Value := Frm_InputSaldoAwalHutang.txtdpp.Value;
          ParamByName('i').Value := Frm_InputSaldoAwalHutang.txtppn.Value;
          ParamByName('j').Value := 0;
          ParamByName('k').Value := 0;
          ParamByName('l').Value := 0;
          ParamByName('n').Value := Frm_InputSaldoAwalHutang.txtnilai.Value;
          ParamByName('o').Value := Frm_Main.txtuser.Caption;
          ParamByName('p').Value := 0;
          ParamByName('q').Value := 0;
          ParamByName('r').Value := 0;
          ParamByName('s').Value := 0;
          ExecSQL;
          dm.con.Commit;
        except
    ...
    Frm_InputSaldoAwalPiutang.LPiutang.Caption := QData.FieldValues['noakun'];
    if Frm_InputSaldoAwalPiutang.ShowModal = mrok then begin
      with TZQuery.Create(Self)do begin
        ...
        try
          ...
          ParamByName('c').Value := FormatDateTime('yyyy-mm-dd',Frm_InputSaldoAwalPiutang.dtpfaktur.Date);
          ParamByName('d').Value := 0;
          ParamByName('e').Value := Frm_InputSaldoAwalPiutang.LKontak.Caption;
          ParamByName('f').Value := Frm_InputSaldoAwalPiutang.LPiutang.Caption;
          ParamByName('g').Value := Frm_InputSaldoAwalPiutang.txtdpp.Value;
          ParamByName('h').Value := Frm_InputSaldoAwalPiutang.txtppn.Value;
          ParamByName('i').Value := 0;
          ParamByName('j').Value := Frm_InputSaldoAwalPiutang.txtnilai.Value;
          ParamByName('k').Value := 0;
          ParamByName('l').Value := 0;
          ParamByName('m').Value := 0;
          ParamByName('n').Value := 0;
          ParamByName('o').Value := Frm_Main.txtuser.Caption;
          ParamByName('p').Value := '';
          ExecSQL;
          dm.con.Commit;
